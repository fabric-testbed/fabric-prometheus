...
These directories may already exist or will be automatically created by the Ansible playbooks.

Prometheus container binds 2 volumes: 
* `prometheus/config` Contains the configuration file `prometheus_config.yml`.
* `prometheus/data` Contains the database and supporting data files that will be created by Prometheus.

Thanos sidecar container binds 2 volumes:
* `thanos/config` Contains the configuration file `object_store_config.yml` which contains the credentials for the S3 storage.
* `prometheus/data` the database and supporting files for prometheus, see above.

Thanos Store Gateway container binds 1 volume.
* `thanos/config` Contains the configuration file `object_store_config.yml` which contains the credentials for the S3 storage. This is the same file as above.

Grafana container binds 3 volumes.
...
* `grafana/custom` Contains custom ini settings.

Nginx container binds 2 volumes.  
`nginx/config/nginx.conf` Contains the Nginx configuration file.
...
Accesible pages are:  
* Grafana, has log in methods `yoursite-hn.fabric-testbed.net:3000` 
* Prometheus info page `yoursite-hn.fabric-testbed.net:9090`
* Thanos info page `yoursite-hn.fabric-testbed.net:10922`  
